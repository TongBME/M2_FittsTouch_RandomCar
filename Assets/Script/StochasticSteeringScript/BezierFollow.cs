    ...
    void DrawCurve()
    {
        ...
        for (int j = 0; j < curveCount; j++)
        {
            for (int i = 1; i <= SEGMENT_COUNT; i++)
            {
                ...
                if (controlPoints[nodeIndex + 1] == null || controlPoints[nodeIndex + 2] == null)
                {
                    continue;
                }

                pixel = CalculateCubicBezierPoint(t, controlPoints[nodeIndex].position, controlPoints[nodeIndex + 1].position, controlPoints[nodeIndex + 2].position);
